...
export const updateSettingsValidator = [
  ...
  body('companyLocation.address').optional({ values: 'falsy' }).trim().isLength({ max: 500 }),
  body('companyLocation.lat').optional({ nullable: true }).isFloat({ min: -90, max: 90 }),
  body('companyLocation.lng').optional({ nullable: true }).isFloat({ min: -180, max: 180 }),
  body('companyLocation.placeId').optional({ values: 'falsy' }).trim(),
